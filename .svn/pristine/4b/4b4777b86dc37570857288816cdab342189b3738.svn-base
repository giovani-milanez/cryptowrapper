...
ByteArray ByteArray::createFromFile(const std::string& filename)
{
	struct stat s;
	bool isFile = (stat (filename.c_str(), &s) == 0);
	if(!isFile)
		throw cryptowrapper::OpenFileException("'"+filename+"' does not exists or is not a file");
...
std::string ByteArray::hex(const ByteArray& ba)
{
	int size = ba.size();
